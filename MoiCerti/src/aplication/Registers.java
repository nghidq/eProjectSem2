...
//Truy van cac dong du lieu trong Table theo id
    ...
    public ResultSet showIDStuReg(String Id) throws SQLException {
        ...
        String sql = "SELECT B.STUDENT_ID, B.STUDENT_NAME, C.CERTIFICATE_ID, C.CERTIFICATE_NAME, A.EXAM_DATE, A.ID_NUM "
                + "FROM STUDENT_CER A, STUDENTS B, CERTIFICATED C, COURSES S "
                + "WHERE B.STUDENT_ID = A.STUDENT_ID AND A.CERTIFICATE_ID = C.CERTIFICATE_ID AND A.EXAM_DATE = S.EXAM_DATE AND B.STUDENT_ID  = '" + Id + "'";
        return DBconnect.loadData(sql);
    }
    public ResultSet showIDStuResult() throws SQLException {
        //cn.getConnect();
        String sql = "SELECT A.STUDENT_ID, B.STUDENT_NAME, A.CERTIFICATE_ID, C.CERTIFICATE_NAME, A.EXAM_DATE, A.ID_NUM, A.AVG_MARK, A.RESULT "
                + "FROM STUDENT_CER A, STUDENTS B, CERTIFICATED C "
                + "WHERE B.STUDENT_ID = A.STUDENT_ID AND A.CERTIFICATE_ID = C.CERTIFICATE_ID ";
        return DBconnect.loadData(sql);
    }
    public ResultSet showStuResultFail() throws SQLException {
        //cn.getConnect();
        String sql = "SELECT A.STUDENT_ID, B.STUDENT_NAME, A.CERTIFICATE_ID, C.CERTIFICATE_NAME, A.EXAM_DATE, A.ID_NUM "
                + "FROM STUDENT_CER A, STUDENTS B, CERTIFICATED C "
                + "WHERE B.STUDENT_ID = A.STUDENT_ID AND A.CERTIFICATE_ID = C.CERTIFICATE_ID ";
        return DBconnect.loadData(sql);
    }
    public ResultSet showIDStuResultPass() throws SQLException {
        //cn.getConnect();
        String sql = "SELECT A.STUDENT_ID, B.STUDENT_NAME, A.CERTIFICATE_ID, C.CERTIFICATE_NAME, A.EXAM_DATE, A.ID_NUM, A.AVG_MARK, A.RESULT "
                + "FROM STUDENT_CER A, STUDENTS B, CERTIFICATED C "
                + "WHERE B.STUDENT_ID = A.STUDENT_ID AND A.CERTIFICATE_ID = C.CERTIFICATE_ID AND A.RESULT IS NOT NULL";
        return DBconnect.loadData(sql);
    }
    public ResultSet showIDStuResultFail() throws SQLException {
        //cn.getConnect();
        String sql = "SELECT A.STUDENT_ID, B.STUDENT_NAME, A.CERTIFICATE_ID, C.CERTIFICATE_NAME, A.EXAM_DATE, A.ID_NUM, A.AVG_MARK, A.RESULT "
                + "FROM STUDENT_CER A, STUDENTS B, CERTIFICATED C "
...
//Xoa 1 dong du lieu vao table
    ...
    public ResultSet showIDCard(String IDCard) throws SQLException {
        DBconnect.getConnect();
        String sql = "SELECT s.STUDENT_NAME, c.CERTIFICATE_NAME, sc.EXAM_DATE, sc.ID_NUM FROM \n"
                + "STUDENTS AS s, STUDENT_CER AS sc, CERTIFICATED AS c\n"
                + "WHERE s.STUDENT_ID=sc.STUDENT_ID AND c.CERTIFICATE_ID=sc.CERTIFICATE_ID AND s.IDENTITY_CARD='" + IDCard + "'";
        return DBconnect.loadData(sql);
    }
